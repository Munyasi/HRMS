            ...
            if(resPatient){
                ...
                async.parallel({
                    ...
                    history: function(callback) {
                        var ds = Insurance.dataSource;
                        var sql = "SELECT (SELECT UserAccount.practitioner_type FROM `UserAccount` WHERE id = PatientHospital.practitioner_id) practitioner_type,(SELECT UserAccount.practitioner_full_name FROM `UserAccount` WHERE id = PatientHospital.practitioner_id) practitioner,PatientHospital.diagnosis_code,PatientHospital.createdAt,Hospital.name,Hospital.town as hospital_town,(SELECT GROUP_CONCAT(Disease.name SEPARATOR ', ') FROM `PatientDisease` JOIN Disease ON Disease.id = PatientDisease.disease_id WHERE diagnosis_code=PatientHospital.diagnosis_code) as diagnosis, (SELECT GROUP_CONCAT(Medicine.name SEPARATOR ', ') FROM `PatientMedicine` JOIN Medicine ON Medicine.id = PatientMedicine.medicine_id WHERE diagnosis_code=PatientHospital.diagnosis_code) as medication FROM PatientHospital JOIN Hospital ON Hospital.id = PatientHospital.hospital_id WHERE patient_id="+params.id;
